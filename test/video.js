...
describe('/video', () => {
  describe('/video/humanReadableTimeCode', () => {
    describe('(<milis>, undefined)', () => {
      casesWithoutMilisAndCompress.forEach((caze) => {
        it(`if (${caze[0]}) > ${caze[1]}`, () => {
          expect(humanReadableTimeCode(caze[0])).to.equal(caze[1]);
        });
      });
    });
    describe('(<milis>, { milis: true })', () => {
      casesWithMilis.forEach((caze) => {
        it(`if (${caze[0]}) > ${caze[1]}`, () => {
          expect(humanReadableTimeCode(caze[0], { milis: true })).to.equal(caze[1]);
        });
      });
    });
    ...
    describe('(<milis>, { milis: true, compress: true })', () => {
      casesWithMilisAndCompress.forEach((caze) => {
        it(`if (${caze[0]}) > ${caze[1]}`, () => {
          expect(humanReadableTimeCode(caze[0], { milis: true, compress: true })).to.equal(caze[1]);
        });
      });
    });
  });
